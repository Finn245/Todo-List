...
function handleFormSubmit(event) {
    ...
    const currentProjectIndex = projects.findIndex(project => project.name === projectName);
    ...
    console.log('Updated project:', projects[currentProjectIndex]);
...
function deleteTask(projectName, taskToDelete) {
    ...
    if (currentProject && currentProject.tasks && currentProject.tasks.length > 0) {
        ...
        const taskIndex = currentProject.tasks.findIndex(task => task === taskToDelete);
...
export function addProjectListeners() {
    ...
    container.addEventListener('click', (event) => {
        ...
        if (projectElement) {
            const projectName = projectElement.querySelector('.projectName').textContent;
